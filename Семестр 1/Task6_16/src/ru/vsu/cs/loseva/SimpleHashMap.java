...
import ru.vsu.cs.loseva.util.dummy.DefaultNotSupportedMap;
import ru.vsu.cs.loseva.util.dummy.DefaultNotSupportedSet;
...
public class SimpleHashMap<K, V> implements DefaultNotSupportedMap<K, V> {
    ...
    public static SimpleHashMap<String, Integer> findAbbreviations(String text) {
        SimpleHashMap<String, Integer> abbreviations = new SimpleHashMap<>(text.length());
        String[] words = text.split("\\s+");

        for (String word : words) {
            if (word.matches("[А-ЯA-Z]{3,5}")) {
